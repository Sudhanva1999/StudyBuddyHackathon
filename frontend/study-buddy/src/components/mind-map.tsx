...
const MindMapNode = ({ data }: { data: { label: string; type?: string; isRoot?: boolean } }) => {
  const nodeStyle: CSSProperties = {
    padding: data.isRoot ? '20px' : '12px',
    background: data.isRoot ? '#3b82f6' : (data.type === 'error' ? '#ef4444' : '#1e40af'),
    color: 'white',
    borderRadius: '50%',
...
>>>>>>> c18754456ad028ef6473435a3171aec9465de989
  ...
  const fetchMindMap = useCallback(async () => {
    ...
    try {
      ...
      setError('Failed to fetch mind map: ' + (err instanceof Error ? err.message : String(err)));
    } finally {
      setLoading(false);
    }
  }, [taskId]);
  ...
  const generateMindMap = async () => {
    ...
    try {
      ...
      setError('Failed to generate mind map: ' + (err instanceof Error ? err.message : String(err)));
    } finally {
      setGenerating(false);
    }
  };
    ...
    for (const branch of data.branches) {
      ...
      if (branch.subbranches) {
        for (const subbranch of branch.subbranches) {
          if (!isValidMindMap({ topic: subbranch.name, branches: [subbranch] })) {
            return false;
          }
        }
      }
  ...
  useEffect(() => {
    ...
    const processBranch = (branch: MindMapBranch, parentId: string, level: number, index: number, total: number) => {
      ...
      if (level > 1) {
        ...
        if (parentNode) {
          ...
          angle = parentAngle - Math.PI / 4 + (index / (total - 1)) * Math.PI / 2;
        }
      }
      ...
      if (branch.subbranches && branch.subbranches.length > 0) {
        branch.subbranches.forEach((subbranch, i) => {
          processBranch(subbranch, branchId, level + 1, i, branch.subbranches!.length);
        });
      }
    };
      ...
      <div className="flex flex-col items-center justify-center h-full gap-4">
        ...
        {debugInfo && (
          <div className="mt-4 p-4 bg-muted rounded-md text-xs overflow-auto max-h-[400px]">
            ...
            <div className="mb-2">
              <strong>Current Task ID:</strong> {debugInfo.currentTaskId || 'None'}
            </div>
            <div className="mb-2">
              <strong>LocalStorage Task ID:</strong> {debugInfo.localStorage?.taskId || 'None'}
            </div>
            <div className="mb-4">
              <strong>Backend Tasks:</strong>
              <pre className="mt-1">{JSON.stringify(debugInfo.backendTasks, null, 2)}</pre>
            </div>
            <div>
              <strong>LocalStorage Results:</strong>
              <pre className="mt-1">{JSON.stringify(debugInfo.localStorage?.results, null, 2)}</pre>
            </div>
          </div>
        )}
      </div>
